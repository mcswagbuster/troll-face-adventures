...
["CsvSheet", "package:csv_sheet/csv_sheet.dart", , A, {
  "^": "",
  CsvSheet0: {
    "^": "Object;_CsvSheet$_contents,_headers,_fakeColumn,_row,hasHeaderRow",
    $index: function(_, index) {
      var t1;
      if (typeof index === "string") {
        if (this.hasHeaderRow !== true)
          throw H.wrapException(P.RangeError$(index + " is not a valid column header"));
        if (!this._headers.containsKey$1(index))
          throw H.wrapException(P.RangeError$(index + " is not a valid column header"));
        index = this._headers.$index(0, index);
        t1 = this._fakeColumn;
        t1.row = index;
      } else {
        t1 = this._fakeColumn;
    ...
    _countQuotes$1: function(str) {
      var t1, quoteIndex, numQuotes, t2, t3;
      t1 = J.getInterceptor$asx(str);
      quoteIndex = t1.indexOf$1(str, "\"");
      for (numQuotes = 0; quoteIndex !== -1;) {
        ++numQuotes;
        t2 = quoteIndex + 1;
        t3 = t1.get$length(str);
        if (typeof t3 !== "number")
          return H.iae(t3);
        if (t2 < t3)
          quoteIndex = t1.indexOf$2(str, "\"", t2);
        else
          break;
      }
      return numQuotes;
    },
    CsvSheet$4$fieldSep$headerRow$lineSep: function(contents, fieldSep, headerRow, lineSep) {
      var allRows, _rows, i, t1, val, i0, t2, allRowsLength, index, numQuotes, index0, tmpRow, rowLength, ind, ind0;
      this._fakeColumn = new A._CsvColumn(this, 0);
      allRows = J.split$1$s(contents, lineSep);
      this.hasHeaderRow = headerRow;
      if (headerRow) {
        if (0 >= allRows.length)
          return H.ioore(allRows, 0);
        _rows = J.split$1$s(allRows[0], fieldSep);
        this._headers = P.HashMap_HashMap(null, null, null, null, null);
        for (i = 0; t1 = _rows.length, i < t1; i = i0) {
          val = _rows[i];
          i0 = i + 1;
          if (H.Lists_indexOf(_rows, val, i0, t1) !== -1)
            throw H.wrapException(P.FormatException$("The header column " + H.S(val) + " appears more than once"));
          t1 = this._headers;
          if (i >= _rows.length)
            return H.ioore(_rows, i);
          t1.$indexSet(0, _rows[i], i);
        }
        t1 = this._headers;
        t2 = new A.CsvRow(null, t1, false);
        if (t1 != null)
          t2.hasHeaders = true;
        this._row = t2;
        this._CsvSheet$_contents = H.setRuntimeTypeInfo([], [[P.List, P.String]]);
        allRowsLength = allRows.length;
        for (index = 1; index < allRowsLength; index = index0) {
          if (index >= allRows.length)
            return H.ioore(allRows, index);
          numQuotes = this._countQuotes$1(allRows[index]);
          for (index0 = index + 1; C.JSInt_methods.$mod(numQuotes, 2) !== 0;) {
            --allRowsLength;
            if (index0 > allRowsLength)
              throw H.wrapException(P.FormatException$("Unterminated quoted field"));
            if (index >= allRows.length)
              return H.ioore(allRows, index);
            t1 = J.$add$ns(allRows[index], lineSep);
            if (index0 >= allRows.length)
              return H.ioore(allRows, index0);
            t1 = J.$add$ns(t1, allRows[index0]);
            if (index >= allRows.length)
              return H.ioore(allRows, index);
            allRows[index] = t1;
            C.JSArray_methods.removeAt$1(allRows, index0);
            if (index >= allRows.length)
              return H.ioore(allRows, index);
            numQuotes = this._countQuotes$1(allRows[index]);
          }
          if (index >= allRows.length)
            return H.ioore(allRows, index);
          tmpRow = J.split$1$s(allRows[index], fieldSep);
          rowLength = tmpRow.length;
          for (t1 = rowLength, ind = 0; ind < rowLength; t1 = t2, ind = ind0) {
            if (ind >= t1)
              return H.ioore(tmpRow, ind);
            numQuotes = this._countQuotes$1(tmpRow[ind]);
            for (ind0 = ind + 1; C.JSInt_methods.$mod(numQuotes, 2) === 1;) {
              --rowLength;
              if (ind0 > rowLength)
                throw H.wrapException(P.FormatException$("Unterminated quoted field"));
              if (ind >= tmpRow.length)
                return H.ioore(tmpRow, ind);
              t1 = J.$add$ns(tmpRow[ind], fieldSep);
              if (ind0 >= tmpRow.length)
                return H.ioore(tmpRow, ind0);
              t1 = J.$add$ns(t1, tmpRow[ind0]);
              if (ind >= tmpRow.length)
                return H.ioore(tmpRow, ind);
              tmpRow[ind] = t1;
              C.JSArray_methods.removeAt$1(tmpRow, ind0);
              if (ind >= tmpRow.length)
                return H.ioore(tmpRow, ind);
              numQuotes = this._countQuotes$1(tmpRow[ind]);
            }
            if (ind >= tmpRow.length)
              return H.ioore(tmpRow, ind);
            t1 = C.JSString_methods.trim$0(J.replaceAll$2$s(tmpRow[ind], "\"", ""));
            t2 = tmpRow.length;
            if (ind >= t2)
              return H.ioore(tmpRow, ind);
            tmpRow[ind] = t1;
          }
          this._CsvSheet$_contents.push(tmpRow);
        }
      } else {
        this._row = new A.CsvRow(null, null, false);
        this._CsvSheet$_contents = H.setRuntimeTypeInfo([], [[P.List, P.String]]);
        allRowsLength = allRows.length;
        for (index = 0; index < allRowsLength; index = index0) {
          if (index >= allRows.length)
            return H.ioore(allRows, index);
          numQuotes = this._countQuotes$1(allRows[index]);
          for (index0 = index + 1; C.JSInt_methods.$mod(numQuotes, 2) !== 0;) {
      ...
      for (; J.get$last$ax(this._CsvSheet$_contents).length !== J.get$first$ax(this._CsvSheet$_contents).length;) {
        t1 = this._CsvSheet$_contents;
        if (0 >= t1.length)
          return H.ioore(t1, 0);
        t1.pop();
      }
    },
    static: {CsvSheet$: function(contents, fieldSep, headerRow, lineSep) {
        var t1 = new A.CsvSheet0(null, null, null, null, null);
        t1.CsvSheet$4$fieldSep$headerRow$lineSep(contents, fieldSep, headerRow, lineSep);
        return t1;
      }}
  },
  _CsvColumn: {
    ...
    $index: function(_, index) {
      ...
      if (typeof index !== "number")
        return index.$sub();
      t2 = index - 1;
      t3 = this.sheet._CsvSheet$_contents;
  ...
  CsvRow: {
    "^": "Object;row,_headers,hasHeaders",
    $index: function(_, index) {
      if (typeof index === "string") {
        if (!this.hasHeaders)
          throw H.wrapException(P.RangeError$(index + " is not a valid column header"));
      ...
      return C.JSNull_methods.$index(this.row, index);
    },
    $indexSet: function(_, index, value) {
      throw H.wrapException(P.UnsupportedError$("Unable to assign a value to a cell."));
    },
    toString$0: function(_) {
      return C.JSNull_methods.toString$0(this.row);
    }
  }
}],
...
["_interceptors", "dart:_interceptors", , J, {
  ...
  Interceptor: {
    ...
    "%": "ArrayBuffer|Blob|CSSStyleSheet|CanvasRenderingContext|CanvasRenderingContext2D|DOMError|File|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedLength|SVGAnimatedLengthList|SVGAnimatedNumber|SVGAnimatedNumberList|StyleSheet"
  },
  JSBool: {
    "^": "Interceptor;",
    toString$0: function(receiver) {
...
["dart.math", "dart:math", , P, {
  ...
  Point: {
    ...
    $sub: function(_, other) {
      ...
      if (typeof t1 !== "number")
        return t1.$sub();
      t2 = new P.Point(t3, C.JSNumber_methods.$sub(t1, t2));
      t2.$builtinTypeInfo = this.$builtinTypeInfo;
      return t2;
    },
    $isPoint: true
...
["foo", "getCrime.dart", , G, {
  ...
  getLocation: function() {
    var t1 = {};
    $.get$countyCo().$indexSet(0, "avon", [52.19166, -1.70734]);
    $.get$countyCo().$indexSet(0, "dedfordshire", [52.0627394, -0.5292091000000028]);
    $.get$countyCo().$indexSet(0, "berkshire", [51.4659863, -1.2814014000000498]);
    $.get$countyCo().$indexSet(0, "birmingham", [52.48624299999999, -1.8904009999999971]);
    $.get$countyCo().$indexSet(0, "cambrige", [52.205337, 0.12181699999996454]);
    $.get$countyCo().$indexSet(0, "cheshire", [53.23263439999999, -2.610315700000001]);
    $.get$countyCo().$indexSet(0, "cleveland", [52.0593166, -0.7768281999999544]);
    $.get$countyCo().$indexSet(0, "cornwall", [50.5036299, -4.652498199999968]);
    $.get$countyCo().$indexSet(0, "derbyshire", [53.122322, -1.5136820999999827]);
    $.get$countyCo().$indexSet(0, "devon", [50.77721349999999, -3.9994609999999966]);
    $.get$countyCo().$indexSet(0, "dorset", [50.7390661, -2.3382345999999643]);
    $.get$countyCo().$indexSet(0, "durham", [54.77525, -1.5848519999999553]);
    $.get$countyCo().$indexSet(0, "glostouchshire", [51.8642449, -2.2381560000000036]);
    $.get$countyCo().$indexSet(0, "herefordshire", [52.0765164, -2.654418200000009]);
    $.get$countyCo().$indexSet(0, "hertfordshire", [51.80978229999999, -0.2376744000000599]);
    $.get$countyCo().$indexSet(0, "lancashire", [53.9690089, -2.627690799999982]);
    $.get$countyCo().$indexSet(0, "leicestershire", [52.6368778, -1.1397591999999577]);
    $.get$countyCo().$indexSet(0, "lincolnshire", [53.2178821, -0.1999702000000525]);
    $.get$countyCo().$indexSet(0, "london", [51.508129, -0.12800500000003012]);
    $.get$countyCo().$indexSet(0, "manchester", [53.479251, -2.2479260000000068]);
    $.get$countyCo().$indexSet(0, "middelsbrough", [54.574227, -1.234956000000011]);
    $.get$countyCo().$indexSet(0, "norfolk", [52.6139686, 0.8864020999999411]);
    $.get$countyCo().$indexSet(0, "northhamptonshire", [52.27299439999999, -0.8755515000000287]);
    $.get$countyCo().$indexSet(0, "nottingham", [52.95477, -1.1580860000000257]);
    $.get$countyCo().$indexSet(0, "oxford", [51.751724, -1.255284999999958]);
    $.get$countyCo().$indexSet(0, "somerset", [51.0587013, -2.949906599999963]);
    $.get$countyCo().$indexSet(0, "sufolk", [52.1872472, 0.9707800999999563]);
    $.get$countyCo().$indexSet(0, "sunderland", [54.906869, -1.3838009999999485]);
    $.get$countyCo().$indexSet(0, "sussex", [51.5153492, -0.17241100000001097]);
    $.get$countyCo().$indexSet(0, "yorkshire", [54.287194, -1.9548698999999488]);
    P.print("helllok jklajdklasjf;lkjdf");
    t1.currentPos_0 = null;
    J.getCurrentPosition$0$x(window.navigator.geolocation).then$2$onError(new G.getLocation_closure(t1), new G.getLocation_closure0());
  },
  calculateClosest: function(lat1, lon1) {
    $.get$countyCo().forEach$1(0, new G.calculateClosest_closure(lat1, lon1));
    P.print($.key);
    P.print($.smallest);
    G.setUpCrime();
  },
  setUpCrime: function() {
    G.getCrimes().then$1(new G.setUpCrime_closure());
  },
  enimNum: function(crime) {
    var t1, t2, t3, t4;
    t1 = {};
    t1.enim_0 = 0;
    t2 = J.getInterceptor$ax(crime);
    t2.forEach$1(crime, new G.enimNum_closure(t1));
    t3 = t1.enim_0;
    t4 = t2.get$length(crime);
    if (typeof t4 !== "number")
      return H.iae(t4);
    $.numEnim = C.JSNumber_methods.toInt$0(C.JSNumber_methods.roundToDouble$0(t3 / t4 / 600));
    t1 = t1.enim_0;
    t2 = t2.get$length(crime);
    if (typeof t2 !== "number")
      return H.iae(t2);
    ...
    P.print(t1);
    $.enemy = Array($.numEnim);
    t1 = new V.SolarSystem($.get$canvas(), null, null, null, null);
    P.scheduleMicrotask(t1.get$start(t1));
    V.addSound("bow.ogg", 65);
    V.addSound("pop1.ogg", 66);
  },
  getCrimes: function() {
    P.print($.key);
    return W.HttpRequest_getString("dataSets/" + H.S($.key) + ".csv", null, null).then$1(new G.getCrimes_closure());
  },
  getLocation_closure: {
    "^": "Closure:19;box_0",
    call$1: function(position) {
      var t1 = this.box_0;
      t1.currentPos_0 = position;
      G.calculateClosest(J.get$latitude$x(J.get$coords$x(position)), J.get$longitude$x(J.get$coords$x(t1.currentPos_0)));
    }
  },
  getLocation_closure0: {
    "^": "Closure:9;",
    call$1: function(error) {
      var t1 = $.get$backUp();
      return G.calculateClosest(t1[0], t1[1]);
    }
  },
  calculateClosest_closure: {
    "^": "Closure:8;lat1_0,lon1_1",
    call$2: function(k, v) {
      var t1, t2;
      P.print("claled for each");
      t1 = J.getInterceptor$asx(v);
      t2 = J.$sub$n(t1.$index(v, 0), this.lat1_0);
      if (typeof t2 !== "number")
        H.throwExpression(P.ArgumentError$(t2));
      t2 = Math.pow(t2, 2);
      t1 = J.$sub$n(t1.$index(v, 1), this.lon1_1);
      if (typeof t1 !== "number")
  ...
  getCrimes_closure: {
    ...
    call$1: function(data) {
      var sheet, crimes, t1, t2;
      sheet = A.CsvSheet$(data, ",", false, "\n");
      crimes = P.LinkedHashMap_LinkedHashMap(null, null, null, null, null);
      t1 = sheet.$index(0, 6);
      t2 = t1.sheet;
      t1 = t1.row;
      t2 = t2._CsvSheet$_contents;
      if (2 >= t2.length)
        return H.ioore(t2, 2);
      t2 = t2[2];
      if (t1 >>> 0 !== t1 || t1 >= t2.length)
        return H.ioore(t2, t1);
      crimes.$indexSet(0, "violence", H.Primitives_parseInt(t2[t1], null, null));
      t1 = sheet.$index(0, 6);
      t2 = t1.sheet;
      t1 = t1.row;
      t2 = t2._CsvSheet$_contents;
      ...
      if (t1 >>> 0 !== t1 || t1 >= t2.length)
        return H.ioore(t2, t1);
      crimes.$indexSet(0, "robbery", H.Primitives_parseInt(t2[t1], null, null));
      t1 = sheet.$index(0, 6);
      t2 = t1.sheet;
      t1 = t1.row;
      t2 = t2._CsvSheet$_contents;
      ...
      if (t1 >>> 0 !== t1 || t1 >= t2.length)
        return H.ioore(t2, t1);
      crimes.$indexSet(0, "theft", H.Primitives_parseInt(t2[t1], null, null));
      t1 = sheet.$index(0, 6);
      t2 = t1.sheet;
      t1 = t1.row;
      t2 = t2._CsvSheet$_contents;
      ...
      if (t1 >>> 0 !== t1 || t1 >= t2.length)
        return H.ioore(t2, t1);
      crimes.$indexSet(0, "burglary", H.Primitives_parseInt(t2[t1], null, null));
      t1 = sheet.$index(0, 6);
      t2 = t1.sheet;
      t1 = t1.row;
      t2 = t2._CsvSheet$_contents;
      ...
      if (t1 >>> 0 !== t1 || t1 >= t2.length)
        return H.ioore(t2, t1);
      crimes.$indexSet(0, "drugs", H.Primitives_parseInt(t2[t1], null, null));
      return crimes;
    }
  }
}],
...
["", "yrsapptestlevel.dart", , V, {
  ...
  istouching: function(x, y) {
    ...
    for (istouching = 0, i = 0; t1 = $.get$platforms(), i < t1.length; ++i) {
      Platform = t1[i];
      t1 = Platform.x;
      t2 = $.sun2.imgwidth;
      if (typeof t2 !== "number")
        return H.iae(t2);
      if (t1 < x + t2) {
        t2 = Platform.imgwidth;
        if (typeof t2 !== "number")
          return H.iae(t2);
        if (x < t1 + t2) {
          t1 = Platform.y;
          t2 = Platform.imgheight;
          if (typeof t2 !== "number")
            return H.iae(t2);
          t1 = y < t1 + t2 && t1 < y;
        } else
  ...
  addSound: function(soundurl, index) {
    var audioContext, xhr;
    audioContext = new (window.AudioContext || window.webkitAudioContext)();
    xhr = new XMLHttpRequest();
    C.HttpRequest_methods.open$2(xhr, "GET", soundurl);
    xhr.responseType = "arraybuffer";
    C.HttpRequest_methods.addEventListener$2(xhr, "load", new V.addSound_closure(index, audioContext, xhr));
    xhr.send();
  },
  keyboard: {
    "^": "Object;_yrsapptestlevel$_key",
    keyboard$0: function() {
      var t1 = H.setRuntimeTypeInfo(new W._EventStream(window, C.EventStreamProvider_keydown._eventType, false), [null]);
      H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new V.keyboard_closure(this)), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
      t1 = H.setRuntimeTypeInfo(new W._EventStream(window, C.EventStreamProvider_keyup._eventType, false), [null]);
      H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new V.keyboard_closure0(this)), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
    },
    static: {keyboard$: function() {
        var t1 = new V.keyboard(P.HashMap_HashMap(null, null, null, P.$int, P.$int));
        t1.keyboard$0();
        return t1;
      }}
  },
  keyboard_closure: {
    "^": "Closure:9;this_0",
    call$1: function(e) {
      var t1 = this.this_0._yrsapptestlevel$_key;
      if (!t1.containsKey$1(J.get$keyCode$x(e)))
        t1.$indexSet(0, e.keyCode, e.timeStamp);
    }
  },
  keyboard_closure0: {
    "^": "Closure:9;this_1",
    call$1: function(e) {
      this.this_1._yrsapptestlevel$_key.remove$1(0, J.get$keyCode$x(e));
    }
  },
  SolarSystem: {
    "^": "Object;canvas,width,height,sun,renderTime",
    start$0: [function(_) {
      var t1, t2, t3, t4, t5, rect, platformscoord, i, z;
      t1 = this.canvas;
      t2 = t1.parentElement;
      t3 = t2.clientLeft;
      t4 = t2.clientTop;
      ...
      if (t2 < 0)
        t2 = -t2 * 0;
      rect = H.setRuntimeTypeInfo(new P.Rectangle(t3, t4, t5, t2), [null]);
      t2 = rect.width;
      this.width = t2;
    ...
    AI$0: function() {
      var neimVis, i, t1;
      for (neimVis = false, i = 0; t1 = $.enemy, i < t1.length; ++i) {
        t1 = t1[i];
        if (t1.isvisable === 1) {
          t1.fire$1("blob");
          neimVis = true;
      ...
      P.Future_Future$delayed(C.Duration_1000000, new V.SolarSystem_AI_closure(this));
      if (neimVis) {
        t1 = $.get$globalfunction()[66];
        if (t1 != null)
          t1.call$0();
      }
    },
    draw$1: [function(_) {
      ...
      this.renderTime = t1;
      if ($.get$k()._yrsapptestlevel$_key.containsKey$1(87))
        $.sun2.jump$0();
      $.sun2.moveleftright$1(0);
      if ($.get$k()._yrsapptestlevel$_key.containsKey$1(68))
        $.sun2.moveleftright$1(1);
      if ($.get$k()._yrsapptestlevel$_key.containsKey$1(65))
        $.sun2.moveleftright$1(-1);
      if ($.get$k()._yrsapptestlevel$_key.containsKey$1(190)) {
        if (!$.FIP) {
          $.sun2.fire$1("Arrow");
          $.FIP = true;
          t1 = $.get$globalfunction()[65];
          if (t1 != null)
            t1.call$0();
        }
      } else
    ...
    drawPlanets$1: function(context) {
      ...
      if (typeof t3 !== "number")
        return t3.$div();
      t1.draw$1(H.setRuntimeTypeInfo(new P.Point(t2 / 2, t3 / 2), [null]));
      for (i = 0; t1 = $.get$platforms(), i < t1.length; ++i) {
        t1 = t1[i];
        t2 = new P.Point(0, 0);
        t2.$builtinTypeInfo = [null];
        t1.draw$1(t2);
      }
      for (i = 0; t1 = $.enemy, i < t1.length; ++i) {
        t1 = t1[i];
        t2 = new P.Point(0, 0);
        t2.$builtinTypeInfo = [null];
        t1.draw$1(t2);
      }
      for (i = 0; t1 = $.get$arrowlist(), i < t1.length; ++i) {
        t1 = t1[i];
        t2 = new P.Point(0, 0);
        t2.$builtinTypeInfo = [null];
        t1.draw$1(t2);
  ...
  PlanetaryBody: {
    "^": "Object;color,orbitPeriod,solarSystem,bodySize,orbitRadius,orbitSpeed,isvisable,x>,y>,speedx,speedy,image,imageleft,imageright,imgheight,imgwidth,gravity,speed,candie,dying,planets",
    isharming$0: function() {
      var isharming, i, t1, Projectile, t2, t3;
      for (isharming = 0, i = 0; t1 = $.get$arrowlist(), i < t1.length; ++i) {
        Projectile = t1[i];
        t1 = Projectile.x;
        t2 = this.x;
        if (t1 < t2) {
          t3 = Projectile.imgwidth;
          if (typeof t3 !== "number")
            return H.iae(t3);
          if (t2 < t1 + t3) {
            t1 = this.y;
            t2 = Projectile.y;
            t3 = Projectile.imgheight;
            if (typeof t3 !== "number")
              return H.iae(t3);
            if (t1 < t2 + t3) {
              t3 = this.imgheight;
    ...
    draw$1: function(p) {
      var newx, t1, t2, newy, pos;
      if (this.imgheight === 0)
        this.imgheight = J.get$height$x(this.imageright);
      if (this.imgwidth === 0)
      ...
      pos = H.setRuntimeTypeInfo(new P.Point(this.x, this.y), [null]);
      if (this.candie && this.isharming$0() === 1) {
        this.isvisable = 0;
        this.x = -20;
        this.y = -20;
      }
      if (this.isvisable === 1)
        this.drawSelf$2($.get$context(), pos);
    },
    drawSelf$2: function(context, p) {
      var t1, t2, t3;
      t1 = this.bodySize;
      if (t1 === 0) {
        t1 = J.get$width$x(this.imageright);
        this.bodySize = t1;
      }
      t2 = p.x;
      if (typeof t2 !== "number")
    ...
    jump$0: function() {
      ...
      if (typeof t3 !== "number")
        return H.iae(t3);
      if (V.istouching(t1, t2 + t3 + 1) === 1)
        this.speedy = -3;
    },
    moveleftright$1: function(direction) {
    ...
    fire$1: function(imgname) {
      var t1, t2, xadjust, arrow;
      t1 = this.image;
      t2 = this.imageright;
  ...
  addSound_closure: {
    "^": "Closure:9;index_0,audioContext_1,xhr_2",
    call$1: function(e) {
      var t1 = this.audioContext_1;
      J.decodeAudioData$1$x(t1, W._convertNativeToDart_XHR_Response(this.xhr_2.response)).then$1(new V.addSound__closure(this.index_0, t1));
    }
  },
  addSound__closure: {
    "^": "Closure:9;index_3,audioContext_4",
    call$1: function(buffer) {
      var t1, t2;
      t1 = $.get$globalfunction();
      t2 = this.index_3;
      t1.length;
      if (t2 >= 100)
        return H.ioore(t1, t2);
      t1[t2] = new V.addSound___doPlaySound(this.audioContext_4, buffer);
    }
  },
  addSound___doPlaySound: {
    "^": "Closure:2;audioContext_5,buffer_6",
    call$0: function() {
      var t1, source;
      t1 = this.audioContext_5;
      source = t1.createBufferSource();
      source.connect(t1.destination, 0, 0);
      source.buffer = this.buffer_6;
      if (!!source.start)
        source.start(0);
      else
        source.noteOn(0);
...
Isolate.$lazy($, "countyCo", "countyCo", "get$countyCo", function() {
...
Isolate.$lazy($, "backUp", "backUp", "get$backUp", function() {
  return [53.479251, -2.2489260000000098];
});
...
Isolate.$lazy($, "arrowlist", "arrowlist", "get$arrowlist", function() {
  return [];
});
Isolate.$lazy($, "platforms", "platforms", "get$platforms", function() {
  return [];
});
Isolate.$lazy($, "globalfunction", "globalfunction", "get$globalfunction", function() {
  return Array(100);
});
// Native classes
...
function dart_precompiled($collectedClasses) {
  ...
  function CsvSheet0(_CsvSheet$_contents, _headers, _fakeColumn, _row, hasHeaderRow) {
    this._CsvSheet$_contents = _CsvSheet$_contents;
    this._headers = _headers;
    this._fakeColumn = _fakeColumn;
    this._row = _row;
    this.hasHeaderRow = hasHeaderRow;
  }
  CsvSheet0.builtin$cls = "CsvSheet0";
  if (!"name" in CsvSheet0)
    CsvSheet0.name = "CsvSheet0";
  $desc = $collectedClasses.CsvSheet0;
  if ($desc instanceof Array)
    $desc = $desc[1];
  CsvSheet0.prototype = $desc;
  function _CsvColumn(sheet, row) {
    this.sheet = sheet;
    this.row = row;
  }
  _CsvColumn.builtin$cls = "_CsvColumn";
  if (!"name" in _CsvColumn)
    _CsvColumn.name = "_CsvColumn";
  $desc = $collectedClasses._CsvColumn;
  if ($desc instanceof Array)
    $desc = $desc[1];
  _CsvColumn.prototype = $desc;
  function CsvRow(row, _headers, hasHeaders) {
    this.row = row;
    this._headers = _headers;
    this.hasHeaders = hasHeaders;
  }
  CsvRow.builtin$cls = "CsvRow";
  if (!"name" in CsvRow)
    CsvRow.name = "CsvRow";
  $desc = $collectedClasses.CsvRow;
  if ($desc instanceof Array)
    $desc = $desc[1];
  CsvRow.prototype = $desc;
  ...
  function getLocation_closure(box_0) {
    this.box_0 = box_0;
  }
  getLocation_closure.builtin$cls = "getLocation_closure";
  if (!"name" in getLocation_closure)
    getLocation_closure.name = "getLocation_closure";
  $desc = $collectedClasses.getLocation_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  getLocation_closure.prototype = $desc;
  function getLocation_closure0() {
  }
  getLocation_closure0.builtin$cls = "getLocation_closure0";
  if (!"name" in getLocation_closure0)
    getLocation_closure0.name = "getLocation_closure0";
  $desc = $collectedClasses.getLocation_closure0;
  if ($desc instanceof Array)
    $desc = $desc[1];
  getLocation_closure0.prototype = $desc;
  function calculateClosest_closure(lat1_0, lon1_1) {
    this.lat1_0 = lat1_0;
    this.lon1_1 = lon1_1;
  }
  calculateClosest_closure.builtin$cls = "calculateClosest_closure";
  if (!"name" in calculateClosest_closure)
    calculateClosest_closure.name = "calculateClosest_closure";
  $desc = $collectedClasses.calculateClosest_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  calculateClosest_closure.prototype = $desc;
  function setUpCrime_closure() {
  }
  setUpCrime_closure.builtin$cls = "setUpCrime_closure";
  if (!"name" in setUpCrime_closure)
    setUpCrime_closure.name = "setUpCrime_closure";
  $desc = $collectedClasses.setUpCrime_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  setUpCrime_closure.prototype = $desc;
  function enimNum_closure(box_0) {
    this.box_0 = box_0;
  }
  enimNum_closure.builtin$cls = "enimNum_closure";
  if (!"name" in enimNum_closure)
    enimNum_closure.name = "enimNum_closure";
  $desc = $collectedClasses.enimNum_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  enimNum_closure.prototype = $desc;
  function getCrimes_closure() {
  }
  getCrimes_closure.builtin$cls = "getCrimes_closure";
  if (!"name" in getCrimes_closure)
    getCrimes_closure.name = "getCrimes_closure";
  $desc = $collectedClasses.getCrimes_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  getCrimes_closure.prototype = $desc;
  ...
  function keyboard(_yrsapptestlevel$_key) {
    this._yrsapptestlevel$_key = _yrsapptestlevel$_key;
  }
  ...
  function SolarSystem_start_closure(this_0) {
    this.this_0 = this_0;
  }
  SolarSystem_start_closure.builtin$cls = "SolarSystem_start_closure";
  if (!"name" in SolarSystem_start_closure)
    SolarSystem_start_closure.name = "SolarSystem_start_closure";
  $desc = $collectedClasses.SolarSystem_start_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  SolarSystem_start_closure.prototype = $desc;
  function SolarSystem_AI_closure(this_0) {
    this.this_0 = this_0;
  }
  SolarSystem_AI_closure.builtin$cls = "SolarSystem_AI_closure";
  if (!"name" in SolarSystem_AI_closure)
    SolarSystem_AI_closure.name = "SolarSystem_AI_closure";
  $desc = $collectedClasses.SolarSystem_AI_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  SolarSystem_AI_closure.prototype = $desc;
  function PlanetaryBody(color, orbitPeriod, solarSystem, bodySize, orbitRadius, orbitSpeed, isvisable, x, y, speedx, speedy, image, imageleft, imageright, imgheight, imgwidth, gravity, speed, candie, dying, planets) {
    this.color = color;
  ...
  PlanetaryBody.prototype.get$x = function(receiver) {
    return this.x;
  };
  PlanetaryBody.prototype.get$y = function(receiver) {
    return this.y;
  };
  function addSound_closure(index_0, audioContext_1, xhr_2) {
    this.index_0 = index_0;
    this.audioContext_1 = audioContext_1;
    this.xhr_2 = xhr_2;
  }
  addSound_closure.builtin$cls = "addSound_closure";
  if (!"name" in addSound_closure)
    addSound_closure.name = "addSound_closure";
  $desc = $collectedClasses.addSound_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  addSound_closure.prototype = $desc;
  function addSound__closure(index_3, audioContext_4) {
    this.index_3 = index_3;
    this.audioContext_4 = audioContext_4;
  }
  addSound__closure.builtin$cls = "addSound__closure";
  if (!"name" in addSound__closure)
    addSound__closure.name = "addSound__closure";
  $desc = $collectedClasses.addSound__closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  addSound__closure.prototype = $desc;
  function addSound___doPlaySound(audioContext_5, buffer_6) {
    this.audioContext_5 = audioContext_5;
    this.buffer_6 = buffer_6;
  }
  addSound___doPlaySound.builtin$cls = "addSound___doPlaySound";
  if (!"name" in addSound___doPlaySound)
    addSound___doPlaySound.name = "addSound___doPlaySound";
  $desc = $collectedClasses.addSound___doPlaySound;
  if ($desc instanceof Array)
    $desc = $desc[1];
